    ...
    pthreadpool_t threadpool)
{
  ...
  for (size_t i = 0; i < num_dims; ++i) {
    xnn_log_error(
      "RS0: i %d offset %d size %d extent %d",
      (int) i, (int)opdata->begins[i],
      (int)opdata->ends[i],
      (int)input_value->shape.dim[i]);
    if (opdata->begins[i] < 0) {
      assert(opdata->begins[i] + input_value->shape.dim[i] >= 0);
      offsets[i] = opdata->begins[i] + input_value->shape.dim[i];
    } else {
      offsets[i] = opdata->begins[i];
    }
    if (opdata->ends[i] < 0) {
      assert(opdata->ends[i] + input_value->shape.dim[i] - offsets[i] + 1 > 0);
      sizes[i] = opdata->ends[i] + input_value->shape.dim[i] - offsets[i] + 1;
    } else {
      assert(opdata->ends[i] - offsets[i] > 0);
